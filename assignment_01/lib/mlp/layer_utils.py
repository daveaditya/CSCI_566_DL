...
class flatten(object):
    ...
    def backward(self, dprev):
        ...
        #############################################################################
        dfeat = dprev.reshape(*(self.meta.shape))
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = None
...
class leaky_relu(object):
    ...
    def backward(self, dprev):
        ...
        #############################################################################
        dfeat = dprev.copy()
        dfeat[feat < 0] *= self.negative_slope
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = None
...
class dropout(object):
    def __init__(self, keep_prob, seed=None, name="dropout"):
        """
        - name: the name of current layer
        - keep_prob: probability that each        # print("dropout forward kept :: ", kept)
 element is kept.
        - meta: to store the forward pass activations for computing backpropagation
    ...
    def forward(self, feat, is_training=True, seed=None):
        ...
        # Store the results in the variable output provided above.                  #
        #############################################################################
        if self.keep_prob == 0 or self.keep_prob == 1:
            kept = np.ones_like(feat)
        else:
            kept = (self.rng.rand(*tuple(feat.shape)) <= self.keep_prob).astype(int)
        if is_training:
            if self.keep_prob != 0:
...
class cross_entropy(object):
    ...
    def forward(self, feat, label):
        ...
        #############################################################################
        one_hot_encoder = lambda no_of_classes, labels: np.identity(no_of_classes)[labels]
        cross_entropy_func = lambda y, y_hat: -np.sum(y * np.log(y_hat))

        label_one_hot = one_hot_encoder(feat.shape[1], label)
        
        sum_of_cross_entropies = 0
        for i in range(feat.shape[0]):
            sum_of_cross_entropies += cross_entropy_func(label_one_hot[i], logit[i])

        loss = sum_of_cross_entropies / feat.shape[0]
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.logit = logit
    ...
    def backward(self):
        ...
        label_one_hot = one_hot_encoder(logit.shape[1], label)
        
        dlogit = (logit - label_one_hot) / logit.shape[0]
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.logit = None
...
def softmax(feat):
    ...
    #############################################################################
    max = np.max(feat,axis=1,keepdims=True) #returns max of each row and keeps same dims
    e_x = np.exp(feat - max) #subtracts each row with its max value
